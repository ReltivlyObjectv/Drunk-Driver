...
int main(void)
{
	...
	system("convert ./images/checkered.jpg ./images/checkered.ppm");
	...
	system("unlink ./images/checkered.ppm");
	return 0;
}
...
void drawStreet(Game& g)
{
	...
	for (int i=0; i<120; i++) {
		glPushMatrix();
		glTranslatef(6.0f, -0.5f, (float)-i*5);
		glTranslatef(-0.5f, -1.5f, (float)-i*5);
		box(0.1, 5.0, 10.0);
		glPopMatrix();
		glPushMatrix();
		glTranslatef(-6.0f, -0.5f, (float)-i*5);
